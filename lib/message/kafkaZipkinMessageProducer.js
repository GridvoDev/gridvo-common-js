...
class MessageProducer {
    constructor({tracer, serviceName = "no-service-name", clientOpts = {}, producerOpts = {}}) {
        ...
        let {
            ...
            clientId = `${this.serviceName}-producer-client`,
            zkOpts = {}
        } = clientOpts;
        let {requireAcks = 1} = producerOpts;
        this.tracer = tracer;
        let self = this;
        this.producerPromise = new Promise((resolve, reject) => {
            self.client = new kafka.Client(
                connectionString, clientId, zkOpts
            );
