...
function zipkinExpressMiddleware({tracer, serviceName = 'unknown'}) {
    return (req, res, next) => {
        tracer.scoped(() => {
            ...
            tracer.recordRpc(`server ${req.method}`);
            tracer.recordAnnotation(new Annotation.LocalAddr({port: 3001}));
            tracer.recordBinary('http.server.recv.url', formatRequestUrl(req));
            let {IS_DEBUG = false} =  process.env;
            if (IS_DEBUG) {
                if(_.isUndefined(req.body) || !req.body){
                    tracer.recordBinary("http.server.req.body", "null");
                }
                else{
                    tracer.recordBinary("http.server.req.body", JSON.stringify(req.body));
                }
            }
            ...
            res.on('finish', () => {
                tracer.scoped(() => {
                    ...
                    tracer.recordBinary('http.server.status.code', res.statusCode.toString());
                });
            });
            next();
        });
